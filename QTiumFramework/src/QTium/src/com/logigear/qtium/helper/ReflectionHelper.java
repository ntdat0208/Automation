...
import com.logigear.qtium.exception.QTiumBaseException;
...
public class ReflectionHelper {
	...
	public static Object invokeFuntionInt(Object comp, String funcName,
			int index) {
		try {
			Object retObj = null;
	...
	public static Object invokeFunctionStr(Object objTarget, String funcName,
			String strInput, boolean ignoreCaseSensitive) {
		try {
			Object retFindNode = null;
			Class<?> mypartypes[] = new Class[1];
			mypartypes[0] = String.class;

			/*Method mtdFindNode = comp.getClass()
					.getMethod(funcName, mypartypes);*/
			...
			myarglist[0] = new String(strInput);
	...
	public static Object invokeFunctionEx(Object objToInvoke,
			String strFuncName, Object... ClassNObjectInputs) {
		...
		try {
			...
			Boolean blIsNoParamFunction = (ClassNObjectInputs == null
					|| (ClassNObjectInputs != null && ClassNObjectInputs.length == 0) ? true
					: false);

			if (objToInvoke == null)
				throw new QTiumBaseException("Invalid object input.");

			if (!(strFuncName != null && strFuncName.length() > 0))
				throw new QTiumBaseException("Invalid function name.");

			if (blIsNoParamFunction == false) {
				if (!(ClassNObjectInputs.length % 2 == 0))
					throw new QTiumBaseException(
							"Invalid number of class/object input.");

				for (int i = 0; i < ClassNObjectInputs.length / 2; i++)
					if (!(ClassNObjectInputs[i] instanceof Class))
						throw new QTiumBaseException("Invalid Class order.");
			...
			if (blIsNoParamFunction == false) {
				...
				for (int i = 0; i < intNumberOfArg; i++)
					clsInputsN[i] = (Class<?>) ClassNObjectInputs[i];

				j = 0;
				for (int i = intNumberOfArg; i < intNumberOfArg * 2; i++)
					objInputsN[j++] = ClassNObjectInputs[i];
			}
			mtToInvoke = objToInvoke.getClass().getMethod(strFuncName,
					(blIsNoParamFunction == false ? clsInputsN : null));
			if (mtToInvoke != null) {
				mtToInvoke.setAccessible(true);

				objValue = mtToInvoke.invoke(objToInvoke,
						(blIsNoParamFunction == false ? objInputsN : null));

			} else {
				objValue = null;
			}

		} catch (QTiumBaseException ex) {
			objValue = null;
		} catch (Exception ex) {
			objValue = null;
		}
	...
	public static Method getMethod(Object objTarget, String methodName, boolean ignoreCaseSensitive, Class<?>... paramTypes){
		Method method = null;
