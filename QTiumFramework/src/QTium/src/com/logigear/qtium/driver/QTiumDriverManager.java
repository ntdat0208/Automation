...
public class QTiumDriverManager {
	...
	/*
	 * public static void setQTiumDriver(DeviceConfigItem deviceConfigItem) { try {
	 * QTiumWebDriver qtiumDriver = QTiumLocalDriverFactory
	 * .createInstance(deviceConfigItem); setQTiumDriver(qtiumDriver); } catch
	 * (InstantiationException e) { // TODO Handle exception e.printStackTrace(); }
	 * catch (IllegalAccessException e) { // TODO Handle exception
	 * e.printStackTrace(); } }
	 */
	...
	protected static boolean isBrowserStillOpen(WebDriver webDriver) {
		try {
			...
			/*
			 * webDriver.getTitle(); return true;
			 */
	...
	public static void closeWebDriver() {
		...
		if (webDriver != null) {
			...
			try {
				 ...
				 * getQTiumDriver().getDeviceName().equalsIgnoreCase(QTiumConstants.DEVICE_IOS)
				 * && getQTiumDriver().getApplicationType().equalsIgnoreCase(QTiumConstants.
				 * APP_TYPE_SAFARI); if(isSafariOniOS){ webDriver.close(); }
				 * 
				 * webDriver.quit();
				 */
				// check WebDriver is quit before close or quit
				RemoteWebDriver remoteWebDriver = (RemoteWebDriver) webDriver;
				if (remoteWebDriver != null) {
					if (remoteWebDriver.getSessionId() != null) {
						boolean isSafariOniOS = getQTiumDriver().getDeviceName()
								.equalsIgnoreCase(QTiumConstants.DEVICE_IOS)
								&& getQTiumDriver().getApplicationType()
										.equalsIgnoreCase(QTiumConstants.APP_TYPE_SAFARI);
						if (isSafariOniOS) {
							webDriver.close();
						}
